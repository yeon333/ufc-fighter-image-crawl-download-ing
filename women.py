...
for anchor in soup.select("div.c-listing-athlete-flipcard__text__back span.c-listing-athlete__name"): #bring soup data and download 50ish img
	...
	for image in images: 
		...
		req = urllib.request.Request(imgUrl, headers={'User-Agent': 'mozilla'})
		try:
		...
		if res.status !=200:
			print("it could have 200 error bro! delete this code before commit")
			continue
		
		with open('%s%s.jpg' %(athele_data, str(pic_count)), 'wb') as out:
